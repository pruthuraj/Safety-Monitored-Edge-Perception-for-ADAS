...
The work grew out of two earlier threads: encountering YOLO during an IoT seminar research project, which made edge-deployed real-time detection concrete, and an ECG digital-twin project, which framed safety as an engineering obligation to reason about when a model should not be trusted and to make that failure observable. This project is their deliberate intersection — an edge detector treated as a component whose untrustworthy regions must be surfaced, not smoothed over.
